...
export const FormInput = ({ type = 'text', title, form, field, mask }: Props) => {
  const inputElement = (
    <input
      ...
      className='truncate text-sm bg-[#BFD2FE33] rounded-[4px] border border-accent h-9 text-secondary pl-3 focus:outline-none focus:ring-0 focus:border-primary group-hover:text-[#1f252d]'
    />
  );
